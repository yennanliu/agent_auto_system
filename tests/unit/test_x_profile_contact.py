...
def test_deobfuscate_ignores_prose_and_mentions():
    """The word 'at' + a literal dot in prose must NOT fabricate an address."""
    from src.automation.tools.contact_harvest import deobfuscate_emails
    # Bare 'at' + literal '.' (real X bio that used to yield at@netlify.evangelist)
    assert deobfuscate_emails("Head of Community at @Netlify . Evangelist") == set()
    assert deobfuscate_emails("based at Google. formerly at Apple.") == set()
    # A plain @mention followed by a sentence must not become an email.
    assert deobfuscate_emails("ping @acme . Great team") == set()


def test_deobfuscate_fullwidth_at():
    from src.automation.tools.contact_harvest import deobfuscate_emails
    assert "owner@shop.com" in deobfuscate_emails("owner＠shop.com")
...
def test_fetch_x_profile_all_fail_returns_empty(mocker):
    ...
    # Also stub the Playwright fallback so the test needs no browser/network.
    mocker.patch.object(m, "_scrape_profile_with_playwright",
                        return_value={"emails": [], "website": "", "location": "",
                                      "bio": ""})
    r = m.fetch_x_profile_contact("acmestudio")
    assert r["emails"] == [] and r["warnings"]


# ── x.com lite-HTML header parsing (Playwright fallback) ────────────────────

# Mirrors x.com's logged-out markup: the bio is the first `dir="auto" text-body`
# block; the follow links mark the header/timeline boundary; a post (with its own
# email) follows and must NOT be harvested.
_XCOM_LITE = """
<div class="css">
  <div dir="auto" class="font-chirp text-text text-body font-normal">
    AI studio for TW SMEs — 聯絡 info [at] acmestudio [dot] tw ·
    <a href="http://acmestudio.tw">acmestudio.tw</a>
  </div>
  <a href="/acmestudio/following">120 Following</a>
  <a href="/acmestudio/verified_followers">4,200 Followers</a>
  <div dir="auto" class="font-chirp text-text text-body font-normal">
    a pinned post — reply to noreply@twitter-post.com maybe
  </div>
</div>
"""


def test_parse_xcom_header_bio_email_website_no_post_leak():
    from src.automation.tools.x_profile_contact_tool import _parse_xcom_header
    r = _parse_xcom_header(_XCOM_LITE, "acmestudio")
    assert r["emails"] == ["info@acmestudio.tw"]        # obfuscated bio email
    assert r["website"] == "http://acmestudio.tw"       # chase-through target
    assert "noreply@twitter-post.com" not in r["emails"]  # post email excluded


def test_fetch_x_profile_falls_back_to_playwright(mocker):
    from src.automation.tools import x_profile_contact_tool as m
    mocker.patch.object(m, "_get_nitter_instances", return_value=["https://dead.test"])
    mocker.patch.object(m, "_fetch", side_effect=Exception("timeout"))
    mocker.patch.object(m, "_scrape_profile_with_playwright", return_value={
        "emails": ["jason@hey.com"], "website": "https://hey.com",
        "location": "", "bio": "reach me at jason@hey.com"})
    r = m.fetch_x_profile_contact("jasonfried")
    assert r["emails"] == ["jason@hey.com"]
    assert r["source"] == "x.com (playwright)"
...
def test_extract_handle_from_url_or_bare():
    ...
    from src.automation.tools.x_profile_contact_tool import _extract_handle
    assert _extract_handle("https://x.com/acmebiz") == "acmebiz"
    assert _extract_handle("https://twitter.com/acmebiz?s=20") == "acmebiz"
    assert _extract_handle("x.com/acmebiz/status/123") == "acmebiz"
